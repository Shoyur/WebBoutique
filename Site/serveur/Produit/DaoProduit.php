...
class DaoProduit {
    ...
    function update(Produit $produit):string {
        $this->connexion = Connexion::getConnexion(); 
        $requete = "";
        $requete = "UPDATE produits SET nom_prod=?, categorie=?, modele=?, fabriquant=?, prix=?, qte_totale=?, chemin_img=? WHERE id_prod=?";
        try {
            $donnees = [
                $produit->getNomProd(),
                $produit->getCategorie(),
            ...
            $stmt = $this->connexion->prepare($requete);
            $stmt->bind_param("ssssdiss", $donnees[0], $donnees[1], $donnees[2], $donnees[3], $donnees[4], $donnees[5], $donnees[6], $donnees[7]);
            $stmt->execute();
